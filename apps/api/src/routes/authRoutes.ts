...
import { actor, publicUser, requireRole, requireSession, issueSession } from "../auth/session.js";
...
authRouter.get("/assignees", requireSession, requireRole("ta"), async (_req, res) => {
  res.json({ data: await prisma.user.findMany({ where: { role: "ta", active: true, membershipStatus: "APPROVED" }, select: publicUser, orderBy: { name: "asc" } }) });
});
